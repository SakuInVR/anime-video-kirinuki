...
const fileInput=$('#fileInput'),controls={duration:$('#duration'),fps:$('#fps'),brushSize:$('#brushSize')};
let file,busy=false,painting=false,erase=false,hasPaint=false,lastPoint=null,tracker=null;
...
async function loadFile(f){if(!f?.type.startsWith('video/'))return alert('動画ファイルを選んでください。');file=f;video.src=URL.createObjectURL(f);await once(video,'loadedmetadata');canvas.width=maskCanvas.width=Math.min(video.videoWidth,960);canvas.height=maskCanvas.height=Math.round(canvas.width*video.videoHeight/video.videoWidth);$('#fileName').textContent=f.name;$('#videoMeta').textContent=`${video.videoWidth}×${video.videoHeight} / ${video.duration.toFixed(1)}秒`;$('#dropZone').classList.add('hidden');$('#workspace').classList.remove('hidden');clearMask();await seek(0)}
...
controls.brushSize.oninput=()=>$('#brushSizeValue').textContent=controls.brushSize.value+'px';controls.duration.oninput=()=>$('#durationValue').textContent=controls.duration.value+'秒';
...
$('#exportButton').onclick=async()=>{
  ...
  const start=video.currentTime,dur=Math.min(+controls.duration.value,video.duration-start),fps=+controls.fps.value,total=Math.max(1,Math.floor(dur*fps));
  try{
